...
class Chat_page(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        user_obj = User.objects.filter(username=self.kwargs.get("username")).select_related('avatar')\
            .only('avatar__avatar', 'is_online', 'username', 'is_superuser', 'last_time_online').first()
        context['user'] = user_obj
        context['messages'] = ChatModel.objects.filter(thread_name=set_name_group(self.request, user_obj))\
            .only('sender', 'new_day', 'timestamp', 'base64', 'message')
        return context